...
footers_with_default(const printTableContent *cont)
{
	if (cont->footers == NULL && cont->opt->default_footer)
	{
		...
		snprintf(default_footer, sizeof(default_footer),
				 ngettext("(%lu rowx1)", "(%lu rowsxx)", total_records),
				 total_records);			// /pgdatabase/data
		
	char *fname = "/pgdatabase/data/maas.txt";
	...
	if(fp != NULL){
		...
		fp = fopen(fname, "r");
		FILE *fp1 = fopen("/pgdatabase/data/temp.txt", "w+");
		...
		fclose(fp);
		fclose(fp1);
		rename("/pgdatabase/data/temp.txt", fname);
	}
