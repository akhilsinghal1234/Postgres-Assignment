...
static backslashResult exec_command_mass(PsqlScanState scan_state, bool active_branch);
			 ...
			 PQExpBuffer previous_buf)
{
	...
	else if (strcmp(cmd, "m") == 0 || strcmp(cmd, "maas") == 0)
		status = exec_command_mass(scan_state, active_branch);
...
/*
 * \mass -- send query and execute each field of result
 */
static backslashResult
exec_command_mass(PsqlScanState scan_state, bool active_branch)
{
	backslashResult status = PSQL_CMD_SKIP_LINE;
	FILE *fp = fopen("/pgdatabase/data/maas.txt", "r");
	if(fp == NULL)
		{
			fp = fopen("/pgdatabase/data/maas.txt", "a");
			fclose(fp);
		}
	else{
		fclose(fp);
		remove("/pgdatabase/data/maas.txt");
	}
